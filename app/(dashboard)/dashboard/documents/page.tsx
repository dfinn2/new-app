                  ...
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                        ...
                        {item.order_status ? item.order_status.charAt(0).toUpperCase() + item.order_status.slice(1) : 'N/A'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {item.amount_paid ? `$${(item.amount_paid / 100).toFixed(2)}` : 'N/A'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      {item.generations_remaining !== undefined && item.generations_remaining !== null ? (
                        <div className="flex items-center">
                          <span className={`font-medium ${
                            item.generations_remaining > 0 ? 'text-green-600' : 'text-gray-500'
                          }`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end space-x-2">
                        ...
                        {item.product?.file_path && (
                          <Button asChild size="sm" variant="outline">
                            <Link href={item.product.file_path} target="_blank" rel="noopener noreferrer">
